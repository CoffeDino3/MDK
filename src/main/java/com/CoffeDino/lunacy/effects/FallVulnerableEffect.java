...
public class FallVulnerableEffect extends MobEffect {

    private static final float BONUS_PERCENT = 0.75f;
    private static final Set<LivingEntity> processing = Collections.newSetFromMap(new WeakHashMap<>());

    public FallVulnerableEffect() {
    ...
    public static void handleLivingDamage(LivingDamageEvent.Post event) {
        ...
        if (!target.hasEffect(ModEffects.FALL_VULNERABLE)) return;
            ...
            if (bonus > 0) {
                ...
                if (target.level() instanceof ServerLevel serverLevel) {
                    serverLevel.sendParticles(ParticleTypes.POOF,
                            target.getX(), target.getY(), target.getZ(), 5, 0.3, 0.1, 0.3, 0.02);
                }
            }
        } finally {
